...
const NotificationCenter = () => {
  ...
  const getTypeIcon = (type) => {
    switch (type) {
      ...
      case 'lab':
        return <TestTubes className="w-4 h-4 text-teal-400" />;
      case 'invoice':
      ...
      >
        ...
        {unreadCount > 0 && (
          <span className="absolute top-1 right-1 flex h-4 w-4">
            ...
            <span className="relative inline-flex rounded-full h-4 w-4 bg-cyan-500 text-[9px] font-bold text-black items-center justify-center font-mono">
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute right-0 mt-3 w-80 sm:w-96 rounded-2xl glass-panel border border-cyan-500/30 shadow-2xl overflow-hidden z-50"
          >
            {/* Header */}
            <div className="flex items-center justify-between px-4 py-3 border-b border-slate-700/60 bg-slate-900/60">
                ...
                notifications.map((n) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className={`text-xs ${!n.isRead ? 'text-slate-100 font-medium' : 'text-slate-300'}`}>
                        {n.message}
                      </p>
                    ...
                    {!n.isRead && (
                      <button
                        ...
                        title="Mark as read"
                        className="text-slate-400 hover:text-cyan-400 p-1 rounded-lg hover:bg-slate-800 shrink-0 cursor-pointer"
                      >
